...
def report_progess(env, nn_output, action, genome, current, info, ref, report, render_game, viewer, obs, speed):
    ...
    if render_game and (current["frame_count"] % speed == 0 or current["done"]):
        ...
        # screen = cv2.resize(obs, dsize=(256 // 8, 224 // 8), interpolation=cv2.INTER_AREA)
        # screen[:,:,1] = 0
        # screen[:,:,2] = 0
        # viewer.imshow(screen)

        # see what the agent sees but high res
        screen = obs
        screen[:,:,1] = 0
        screen[:,:,2] = 0
        viewer.imshow(screen)
...
def eval_genome(
    ...
    # reduce screen resolution by a factor of 8 for each dimension without much loss of quality
    ...
    while not current["done"]:
        # resize SNES otuput resolution and convert to a 1d array
        screen = cv2.resize(obs, dsize=(inx, iny), interpolation=cv2.INTER_AREA)
        screen = screen[:, :, 0]  # keeps only red channel
        screen = screen.flatten()

        # feed the network the current state of the game and get its output
        nn_output = net.activate(screen)
