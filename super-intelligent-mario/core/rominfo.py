...
"""
Feito por Fabrício Olivetti
LEVEMENTE ADAPTADO por mim:
    - coleção de extended sprites
    - tratar itens que podem ser carregados como '2' no array de inputs (afinal, nao sao inimigos)
...
def getSprites(ram):
    ...
    for slot in range(12):
        ...
        if status != 0:
            ...
            if spriteSize == 0:
                size = 4
            sprites.append({"x": spriteX, "y": spriteY, "size": size, "id": spriteId, "status": status})
...
def getTile(dx, dy, ram):
    ...
    #print(0x1F000 + np.int(np.floor(x / 16) * 432 + y * 16 + x % 16))
            ...
            if distx <= 8 and disty <= 8:
                ...
                for s1, s2 in product(range(size), repeat=2):
                    if withinLimits(j, s1, s2, radius, maxlen):
                        # status >=8 menas sprites are "alive" and can be interacted with
                        if int(sprites[i]["status"]) >= 8:
                            # stationary, carryable or carried
                            if int(sprites[i]["status"]) == 9 or int(sprites[i]["status"]) == 11:
                                inputs[j + s1 * (radius * 2 + 1) + s2] = 2
                            else:
                                # treat as dangerous
